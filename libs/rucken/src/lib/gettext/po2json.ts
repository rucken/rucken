...
export function parse(buffer, options) {
  ...
  for (const property in defaults) {
    options[property] =
      'undefined' !== typeof options[property]
        ? options[property]
        : defaults[property];
  }
  ...
  if (options.format === 'mf') {
    ...
    if (options.fullMF) {
      return options.stringify
        ? JSON.stringify(result, null, options.pretty ? '   ' : null)
        : result;
    }
    ...
    return options.stringify
      ? JSON.stringify(mfTranslations, null, options.pretty ? '   ' : null)
      : mfTranslations;
  }

  // Parse the PO file
  const parsed = gettextParser.po.parse(buffer, defaults.charset);
...
export function parseFileSync(fileName, options) {

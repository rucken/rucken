...
function _sort(
  ...
  const {
    ...
    segmentCompareFn: (a: string, b: any) => a.localeCompare(b),
    ...options,
  };
  ...
  function parse(path: { [x: string]: any }) {
    const original = path;
